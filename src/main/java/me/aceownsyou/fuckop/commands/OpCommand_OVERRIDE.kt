...
class OpCommand_OVERRIDE : CommandExecutor {
    override fun onCommand(sender: CommandSender, cmd: Command, label: String, args: Array<out String>): Boolean {
        ...
        if(fetchedPlayer.isOp) {
            sender.sendMessage("${ChatColor.WHITE}\"${fetchedPlayer.name}\" ${ChatColor.RED}is already an operator!");
            return true;
        }

        fetchedPlayer.isOp = true;

        Bukkit.broadcastMessage("${ChatColor.WHITE}\"${fetchedPlayer.name}\" ${ChatColor.GREEN}now has ${ChatColor.GREEN}${ChatColor.BOLD}OPERATOR ${ChatColor.GREEN}status.")
        return true;
    }
}